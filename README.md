...
> **Revised 2026-09-16 — independent model audit applied.** Before these outputs were reused in follow-on work, every cost item was re-derived from primary sources instead of being taken on trust. Twenty errors were found and fixed, and the headline figures changed: break-even against managed SIEM 44.2 → **62.0 GB/day**, 80% interval 37–56 → **37–64 GB/day**, 3-year contract 123.3 → **157.0 GB/day**. **Against Splunk at public list price, the direction of the conclusion reversed** — tiered self-hosting is cheaper across the entire range. Details: [`docs/ERRATA.md`](docs/ERRATA.md) E-02 and [`model_audit/AUDIT_REPORT.md`](model_audit/AUDIT_REPORT.md) (Korean).
...
| Condition (5-year contract, vs. managed SIEM) | Recommendation |
|---|---|
| Below ~37 GB/day | **Buy (managed SIEM).** Labor and infrastructure costs exceed the service fee |
| 37–64 GB/day | **Depends.** The answer shifts with in-house capability and whether servers are bought on commitment |
| Above ~64 GB/day | **Build may win** — but only under one condition |

**That condition turned out to be the most important finding in the project.** Self-hosting is only competitive if the storage is *tiered* — that is, if older data is moved to cheaper storage instead of sitting on expensive high-performance disks. Without tiering, self-hosting lost to managed SIEM across the entire range examined (5–200 GB/day), and lost to Splunk (list price) above roughly 32 GB/day.

There is also a second condition that is easy to miss: **contract length**. On a 3-year contract the break-even point against managed SIEM moves to about 157 GB/day — far outside the range most mid-sized organizations operate in.

**Against Splunk, the discount decides.** At public list price, tiered self-hosting is cheaper than Splunk across the entire range. Splunk becomes cheaper only with a discount of roughly 53% or more at a starting volume of 50 GB/day, or roughly 21% or more at 100 GB/day. Actual Korean contract prices are not public and could not be checked.

The 37–64 GB range varies only price and effort assumptions. **Changing structural assumptions as well** — whether existing logs are migrated, whether the product can search logs held in object storage, how many admins Splunk needs — **widens it to roughly 25–99 GB.**
...
This range (₩93M–145M per person) **contains** the labor cost figure derived independently from national wage statistics (2026 edition: ₩102.0M–133.2M). A number obtained from statistics was confirmed by actual contracts — an unplanned cross-validation.
...
*Each line is one option. The thick red line is self-hosting without tiering — note that it is never the lowest line at any point (it always sits above managed SIEM). The shaded band is the focus range for mid-sized organizations.*

**Of the six comparisons calculated, three produced no crossing at all.** Self-hosting without storage tiering lost to managed SIEM across the entire range, and tiered self-hosting beat Splunk (list price) across the entire range. Those results were reported as-is rather than adjusted.
...
→ *We have a number: 62.0 GB (44.2 GB before the model audit). But it rests on assumptions, some of which have no source. How much can it be trusted?*
...
*The longest bar moves the answer roughly fourteen times as much as the shortest. Blue means the assumption was set to its low end, red to its high end — not "optimistic" and "pessimistic," since the direction reverses depending on the item.*

**This ranking exposed a problem.** The single most influential input — annual software price escalation — had no source at all. The input that moves the answer most (42.2 GB in the current model) was resting on a guess.
...
The correction moved the break-even point from 60.2 GB to 44.2 GB in the model as it stood then. After the later model audit corrected the cost structure, the current figure is 62.0 GB.
...
The correct statement is therefore not "the break-even point is 62.0 GB" but **"approximately 49 GB, falling between 37 and 64 GB with 80% confidence."** That interval holds structural assumptions fixed (see The Short Answer).
...
**Its central finding is a constraint, not a recommendation:** self-hosting without storage tiering lost to managed SIEM across the entire range examined. The proposal states this plainly rather than presenting a balanced-sounding conclusion — and equally states the result that is unfavorable to the product: at list price, tiered self-hosting is cheaper than Splunk across the entire range.
...
*Moving 70% of data to object storage cuts five-year cost by 39% at 50 GB/day. The effect grows with scale.*
...
*In the commercial options, the license block is about half the bar and the storage block is thin. In self-hosting, storage is the largest component.*

License costs are **about 51% of the total** for commercial products — the largest block — so a storage saving of ₩47M shrinks to 2.7% of a ₩1,755M total. (Before the model audit this was computed as 87% and 0.07%.)

Rather than present the total and let the customer discover this later, the proposal states it directly and relocates the argument to where the evidence actually supports it: **storage design matters enormously for self-hosted deployments, and shows up as reduced local hardware requirements (down about 93%) for commercial ones.**

---

## Errors Found and Corrected

Five errors were found and fixed during the work, and one more batch was found by a separate audit after completion (row 6). They are listed because **the process of finding them is part of what the project demonstrates** — a calculation that has never been challenged is not a verified calculation.
...
| 6 | Model audit (2026-09-16) found 20 errors — Splunk price source (subscription) did not match the model structure (self-managed); two years of data assumed present from year one; object-tier copies, server counts and sizing; missing Splunk replication, implementation and admin labor; outdated wage table | Break-even vs. managed SIEM 44.2 → 62.0 GB; direction vs. Splunk reversed | Re-deriving every cost item from primary sources before reuse ([ERRATA E-02](docs/ERRATA.md)) |
...
├── tests/                     141 tests
├── model_audit/               Independent model audit (2026-09-16; report, per-phase evidence, calculator)
...
3. **Korea Software Industry Association**, 2026 Software Engineer Average Wage — fully-loaded labor cost
...
30. **Splunk End Customer Pricelist (EMEA)** — self-managed term-license tiers submitted by a reseller to UK G-Cloud 14 (added in the 2026-09-16 audit)
...
Third-party market research (17–30) was used only where vendors withhold pricing. Where several independent sources converge on a similar figure, that convergence is noted; where a single source stands alone, the value is graded as an assumption and tested in the sensitivity analysis.
